...
const PLAN_KEY = "gp_weekly_plan_v2";
const LOGS_KEY = "gp_progress_logs_v2";
...
export function useWorkoutStore() {
  ...
  const weeklyStats = useMemo(() => {
    ...
    const uniqueMuscleGroups = new Set(thisWeekLogs.map(log => log.muscleGroup)).size;
    
    return {
      totalWorkouts,
      uniqueMuscleGroups,
      consistency: totalWorkouts >= 3 ? 'Excellent' : totalWorkouts >= 1 ? 'Good' : 'Needs Improvement'
    };
  }, [logs]);
